...
public interface HistoricoMedicacaoRepository extends JpaRepository<HistoricoMedicacao, Integer>{
	...
	@Query(value = "SELECT distinct hm.id,m.nome_medicacao,hm.dosagem,hm.data_aplicacao FROM gerenciadorpecuario.medicacao as m inner join gerenciadorpecuario.historico_medicacao as hm ON hm.medicacao_id = m.id and hm.animal_id=?1", nativeQuery = true)
