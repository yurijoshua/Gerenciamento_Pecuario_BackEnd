...
public interface StatusRacaoAndLoteRepository extends JpaRepository<StatusRacaoAndLote, Integer>{
	...
	@Query(value = "SELECT sbr.id,r.ingredientes,r.data_criacao FROM gerenciadorpecuario.racao as r inner join gerenciadorpecuario.status_racao_and_lote as sbr ON sbr.tempo_final = '' and racao_id = r.id and sbr.lote_id  = ?1", nativeQuery = true)
